  ...
  document
    ...
    .forEach((element) => {
      const child = {
        ...
        spacingX: Number(element.getAttribute('spacingx')) || 0,
        spacingY: Number(element.getAttribute('spacingy')) || 0,
        dimensions: reactive({
          top: 0,
          right: 0,
          bottom: 0,
      ...
      child.reposition = () => {
        ...
        function topBottom() {
          return target.top + target.height + child.spacingY;
        }

        function bottomTop() {
          return target.top - child.dimensions.height - child.spacingY;
        }

        function leftLeft() {
          return target.left + child.spacingX;
        }

        function rightRight() {
          return (
            target.left + target.width - child.dimensions.width - child.spacingX
          );
        }
        ...
        if (child.vertical === 'center') {
          ...
          if (topBottom() + child.dimensions.height > window.innerHeight) {
            child.el.style.top = `${bottomTop()}px`;
          } else {
            child.el.style.top = `${topBottom()}px`;
          }
        ...
        if (child.horizontal === 'center') {
          ...
          if (
            ...
            child.el.style.left = `${rightRight()}px`;
          } else {
            child.el.style.left = `${leftLeft()}px`;
          }
